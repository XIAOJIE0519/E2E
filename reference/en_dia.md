...
Trains an Elastic Net-regularized logistic regression model using
...
- tune:
  ...
  `TRUE`) or use a fixed value (if `FALSE`). `alpha` is fixed at 0.5 for
  Elastic Net.
...
representing the trained Elastic Net model.
